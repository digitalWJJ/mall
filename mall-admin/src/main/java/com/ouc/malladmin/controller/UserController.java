...
public class UserController {
    @Resource(type = com.ouc.malladmin.service.impl.UserServiceImpl.class)
    UserService userService;
    @PostMapping("/adduser")
    public Result adduser(@RequestBody User user){
        if(userService.adduser(user) == 0) return Result.result(500,"添加用户失败",null);
        else return Result.result(200,"添加用户成功",null);
    }
    @PostMapping("/updateuser")
    public Result updateuser(@RequestBody User user){
        if(userService.updateuser(user) ==0) return Result.result(402,"更新用户信息失败",null);
    ...
    public Result viewusers(@RequestBody PageParam pageParam){
        ...
        if(usersList.isEmpty()) return Result.result(402,"获取用户列表失败,用户列表为空",null);
    ...
    public Result viewuser(@PathVariable int id){
        ...
        if(user==null)return Result.result(402,"获取用户信息失败，不存在该用户",null);
